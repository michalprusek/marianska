...
class EditBookingGuests {
  ...
  _collectGuestType(type, guestNames) {
    ...
    if (firstNames.length !== lastNames.length) {
      console.warn(
        `[EditBookingGuests] Mismatched input counts for ${type}: firstNames=${firstNames.length}, lastNames=${lastNames.length}`
      );
    }
    ...
    for (let i = 0; i < minLength; i++) {
      ...
      if (firstName && lastName) {
        ...
        if (isBulk) {
          ...
          if (guestIndexMatch) {
            ...
            const toggleInput = document.getElementById(
              `bulk${typeCap}${guestIndex}GuestTypeToggle`
            );
        ...
        // FIX 2025-12-05: Extract roomId from input ID for server validation
