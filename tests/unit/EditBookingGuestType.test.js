...
describe('EditBookingComponent - updateTotalPrice', () => {
    ...
    beforeEach(() => {
        ...
        component = {
            ...
            perRoomDates: new Map([
                ['12', { startDate: '2025-06-10', endDate: '2025-06-12' }]
            ]),
            settings: {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(document, 'querySelectorAll').mockImplementation((selector) => {
            if (selector.includes('adult') && selector.includes('FirstName')) {
                return [mockInputs['room12AdultFirstName1'], mockInputs['room12AdultFirstName2']].map(input => ({
                    ...input,
                    dataset: { guestType: 'adult' },
                }));
            }
            if (selector.includes('adult') && selector.includes('LastName')) {
                return [mockInputs['room12AdultLastName1'], mockInputs['room12AdultLastName2']].map(input => ({
                    ...input,
                    dataset: {},
                }));
            }
            ...
            _collectGuestType(type, guestNames) {
                ...
                for (let i = 0; i < minLength; i++) {
                    ...
                    if (firstName && lastName) {
                        ...
                        // FIX: Extract roomId
                        const roomIdForGuest = isBulk ? null : (inputId.match(/room(\d+)/) ? inputId.match(/room(\d+)/)[1] : null);
