...
class AdminPanel {
    ...
    setupEventListeners() {
        ...
        // Forms
        document.getElementById('blockDateForm').addEventListener('submit', async (e) => await this.handleBlockDate(e));
        document.getElementById('addCodeForm').addEventListener('submit', async (e) => await this.handleAddCode(e));
        document.getElementById('christmasPeriodForm').addEventListener('submit', async (e) => await this.handleChristmasPeriod(e));
        document.getElementById('roomConfigForm').addEventListener('submit', async (e) => await this.handleRoomConfig(e));
        document.getElementById('priceConfigForm').addEventListener('submit', async (e) => await this.handlePriceConfig(e));
        document.getElementById('bulkPriceConfigForm').addEventListener('submit', async (e) => await this.handleBulkPriceConfig(e));
        document.getElementById('changePasswordForm').addEventListener('submit', async (e) => await this.handleChangePassword(e));
        document.getElementById('editBookingForm').addEventListener('submit', async (e) => await this.handleEditBooking(e));
        document.getElementById('emailTemplateForm').addEventListener('submit', async (e) => await this.handleEmailTemplate(e));
    ...
    async loadBlockedDates() {
        ...
        console.log('Loading blocked dates:', blockedDates);
        ...
        const sortedGroups = Object.values(blockageGroups).map(group => {
            group.dates.sort();
            return {
                ...group,
        ...
        if (sortedGroups.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: var(--gray-500);">Žádné blokované termíny</p>';
            return;
        }
                ...
                <div style="flex: 1;">
                    ...
                    <div style="display: flex; align-items: center; flex-wrap: wrap; gap: 0.3rem; margin-bottom: 0.5rem;">
                        ...
                        ${group.roomsList.length > 0
                            ? group.roomsList.map(roomId => this.createRoomBadge(roomId, true)).join('')
    ...
    async handleBlockDate(e) {
        ...
        const roomsText = selectedRooms.length > 0 ? `${selectedRooms.length} pokojů` : 'všechny pokoje';
    ...
    async handleAddCode(e) {
        ...
        try {
            ...
            console.log(`Kód "${newCode}" byl přidán. Celkem kódů: ${settings.christmasAccessCodes.length}`);
        } catch (error) {
    ...
    async removeCode(code) {
        try {
            ...
            const originalCount = settings.christmasAccessCodes ? settings.christmasAccessCodes.length : 0;
            ...
            if (originalCount > newCount) {
                ...
                console.log(`Kód "${code}" byl úspěšně odstraněn. Počet kódů: ${originalCount} → ${newCount}`);
            } else {
    ...
    async loadChristmasPeriods() {
        ...
        if (periods.length === 0) {
            container.innerHTML = '<p style="color: var(--gray-500); text-align: center; padding: 2rem;">Žádná vánoční období nejsou nastavena</p>';
            return;
        }
    ...
    async handleChristmasPeriod(e) {
        ...
        try {
            ...
            const newPeriod = { start: startDate, end: endDate, year: year };
            ...
            console.log(`Přidáno vánoční období: ${startDate} - ${endDate} (${year})`);
        } catch (error) {
    ...
    async removeChristmasPeriod(index) {
        try {
            ...
            console.log(`Odstraněno vánoční období: ${startDate} - ${endDate} (${year})`);
        } catch (error) {
    ...
    async loadRoomConfig() {
        ...
        rooms.forEach(room => {
            ...
            if (typeSelect) typeSelect.value = room.type;
        });
    }
    ...
    async handleRoomConfig(e) {
        ...
        roomIds.forEach(id => {
            ...
            const type = document.getElementById(`room${id}_type`).value;
            newRooms.push({
                id: id,
                name: `Pokoj ${id}`,
                type: type,
    ...
    async loadPriceConfig() {
        ...
        // Load UTIA prices
        document.getElementById('utia_small_base').value = prices.utia.small.base;
        document.getElementById('utia_small_adult').value = prices.utia.small.adult;
        document.getElementById('utia_small_child').value = prices.utia.small.child;
        document.getElementById('utia_large_base').value = prices.utia.large.base;
        document.getElementById('utia_large_adult').value = prices.utia.large.adult;
        document.getElementById('utia_large_child').value = prices.utia.large.child;

        // Load external prices
        document.getElementById('external_small_base').value = prices.external.small.base;
        document.getElementById('external_small_adult').value = prices.external.small.adult;
        document.getElementById('external_small_child').value = prices.external.small.child;
        document.getElementById('external_large_base').value = prices.external.large.base;
        document.getElementById('external_large_adult').value = prices.external.large.adult;
        document.getElementById('external_large_child').value = prices.external.large.child;
    }
    ...
    async handlePriceConfig(e) {
        ...
        const prices = {
            utia: {
                small: {
                    base: parseInt(document.getElementById('utia_small_base').value),
                    adult: parseInt(document.getElementById('utia_small_adult').value),
                    child: parseInt(document.getElementById('utia_small_child').value)
                },
                large: {
                    base: parseInt(document.getElementById('utia_large_base').value),
                    adult: parseInt(document.getElementById('utia_large_adult').value),
                    child: parseInt(document.getElementById('utia_large_child').value)
                }
            },
            external: {
                small: {
                    base: parseInt(document.getElementById('external_small_base').value),
                    adult: parseInt(document.getElementById('external_small_adult').value),
                    child: parseInt(document.getElementById('external_small_child').value)
                },
                large: {
                    base: parseInt(document.getElementById('external_large_base').value),
                    adult: parseInt(document.getElementById('external_large_adult').value),
                    child: parseInt(document.getElementById('external_large_child').value)
                }
            }
        };
    ...
    async loadBulkPriceConfig() {
        ...
        // Load bulk pricing values
        document.getElementById('bulk_base_price').value = bulkPrices.basePrice;
        document.getElementById('bulk_utia_adult').value = bulkPrices.utiaAdult;
        document.getElementById('bulk_utia_child').value = bulkPrices.utiaChild;
        document.getElementById('bulk_external_adult').value = bulkPrices.externalAdult;
        document.getElementById('bulk_external_child').value = bulkPrices.externalChild;
    }
    ...
    async loadEmailTemplate() {
        ...
        document.getElementById('emailSubject').value = emailSettings.subject || 'Potvrzení rezervace - Chata Mariánská';
        document.getElementById('emailTemplate').value = emailSettings.template || document.getElementById('emailTemplate').value;
    }
